...
COLORS = {
    "bg_app" : "#404168",
    "bg_text_field" : "#56577B",
    "login_text": "#EFF7FA",
    "login_text_active": "#8B95A3",
    "base_button" : "#5F607E",
    "base_button_active" : "#25253B",
    "text_delete": "#2E0404", 
    "text_delete_bg": "#A02727",      
    "selection": "#43B556",      
    "selection_txt": "#1A390A",
    "border": "#202225"
}

FONTS = {
    "base": ("Lucida console", 15),
    "thin_base": ("Lucida console", 15),
    "small": ("Lucida console", 10)
}
...
class WindowManager:
    ...
    def run(self):
        try:
            ...
            print(f"reading server response Error: {e}")
        ...
        try:
            ...
            print(f"reading server notification Error: {e}")
    ...
    def ensure_connection(self):
        if not self.client.socket or self.client.socket._closed:
    ...
    def handle_login(self, username, password):
        if not self.ensure_connection():
            messagebox.showerror("Error", "Lost connection to server and cannot reconnect!")
            return
...
class LoginFrame(tk.Frame):
    def __init__(self, root, menager):
        ...
        self.login_input = tk.StringVar(self)
        self.password_input = tk.StringVar(self)

        self.login_box = tk.Entry(center_frame, textvariable=self.login_input,font=FONTS["base"], width=16, fg=COLORS["login_text"], background=COLORS["bg_text_field"])
        self.password_box = tk.Entry(center_frame,show="*", textvariable=self.password_input, font=FONTS["base"], width=16, fg=COLORS["login_text"], background=COLORS["bg_text_field"])        
        self.login_box.pack(padx = 5, pady = 5)
        self.password_box.pack(padx = 5, pady = 5)

        login_func = lambda: menager.handle_login(self.login_input.get(), self.password_input.get())
        register_func = lambda: menager.handle_registration(self.login_input.get(), self.password_input.get())
...
class MenuFrame(tk.Frame):
    def __init__(self, root, menager):
        ...
        new_chat_entry = tk.Entry(chat_creator_frame, textvariable=self.chat_name_input,font=FONTS["thin_base"], width=10, background=COLORS["bg_text_field"])
...
class ChatFrame(tk.Frame):
    def __init__(self, root, menager):
        ...
        self.chat_name_label = tk.Label(
            top_frame,
            text="Wybierz czat", # Tekst domyślny
            bg=COLORS["bg_app"],
            fg=COLORS["login_text"],
            font=FONTS["base"]
        )
        ...
        del_user_button =tk.Button(
            ...
            fg=COLORS["text_delete"],
            bg=COLORS["text_delete_bg"],
            activebackground=COLORS["text_delete"],
            font=FONTS["small"],
